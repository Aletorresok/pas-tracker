...
- [ ] **`src/components/dashboard/CobrosPendientesCard.jsx` (Tech Debt de Consistencia Visual):** Se definen componentes internos auxiliares repetidos como `Badge` de manera local[cite: 61], patrón que también se repite en `MisPendientesCard.jsx`[cite: 63]. Conviene centralizar estos componentes estéticos menores en un archivo compartido para evitar duplicación de código en la UI.
...
- [ ] **`src/components/portal/PortalCasoCard.jsx` y `NuevoCasoModal.jsx` (Duplicidad de Lógica de Email):** Ambos componentes contienen lógica idéntica para procesar arreglos de archivos, subirlos a Supabase Storage y disparar plantillas de EmailJS[cite: 68, 69]. Esta lógica debería modularizarse en una única función utilitaria compartida para evitar la repetición de código.
- [ ] **`src/context/ThemeContext.jsx` (Coherencia Arquitectónica):** Se observa que este proveedor de contexto es ignorado por el módulo del portal externo (`Portal.jsx` y sus componentes), el cual implementa su propia gestión de temas aislada en `portalTheme.js`[cite: 24, 72]. Conviene unificar ambos sistemas bajo este único `ThemeContext` para estandarizar la capa visual de toda la aplicación.
...
- [ ] **`src/utils/formatters.js` y `src/utils/carpeta.js` (Funciones Duplicadas):** Se observa que funciones auxiliares críticas como `formatoFecha` y verificaciones de permisos del sistema de archivos (`verificarPermiso`) se encuentran duplicadas o re-declaradas de forma independiente tanto en archivos de utilidades (`formatters.js`, `carpeta.js`) como dentro de algunos componentes específicos (`generarEscrito.js`)[cite: 76, 79, 80]. Conviene centralizar estas funciones de soporte en un único módulo para mantener el principio DRY (Don't Repeat Yourself).
- [ ] **`src/utils/formatters.js` y `src/utils/carpeta.js` (Funciones Duplicadas):** Se observa que funciones auxiliares críticas como `formatoFecha` y verificaciones de permisos del sistema de archivos (`verificarPermiso`) se encuentran duplicadas o re-declaradas de forma independiente tanto en archivos de utilidades (`formatters.js`, `carpeta.js`) como dentro de algunos componentes específicos (`generarEscrito.js`)[cite: 76, 79, 80]. Conviene centralizar estas funciones de soporte en un único módulo para mantener el principio DRY (Don't Repeat Yourself).