...
*   **Rutina** (pestaña nueva): Diaria / Semanal / Mensual / Anual. Bloques con horario, prioridad (imprescindible / importante / postergable), checklist y accesos directos; listas automáticas (clientes sin novedades hace 15 días, documentación pendiente, ofrecimientos sin respuesta, escritos colgados); "Preparar audiencia" el día anterior. **Días de escuela:** rango de fechas + horario; la rutina se reacomoda según prioridad. **Anual:** objetivos del mes, trimestre, semestre y año; línea "Objetivo anual" en Hoy; los medibles también en Análisis. Cada objetivo medible se ve como **anillo coloreado** (hecho vs. lo que falta en gris), con una **marca de "dónde deberías estar hoy"** (verde si vas a tiempo, naranja si vas un poco atrás, rojo si vas muy atrás) y una **frase útil** calculada ("a este ritmo cerrás el año en…", "hay 4 PAS dormidos que derivaban"). KPIs propuestos: PAS activos derivando (90 días), honorarios cobrados, casos nuevos, casos cobrados, tiempo hasta el cobro (meta a la baja), rutina cumplida. Los objetivos sin número se tildan a mano.
...
### 2026-09-26 — ¿Quién tiene la pelota?, ¿conviene mediar? y proyección "si todo sale bien" (SQL 23 ejecutado)
*   **¿Quién tiene la pelota?** (`utils/pelota.js`): Doc. pendiente = esperando al cliente; Iniciado y Con ofrecimiento = vos; Reclamado y Esperando pago = la compañía; Mediación y juicio, aparte. Una próxima acción vencida la pasa a vos. **Casos:** chips "Te toca a vos", "Esperando al cliente", "Esperando a la compañía", "Mediación o juicio". **Hoy:** "N te tocan a vos" en el KPI En gestión (lleva a Casos). **Análisis → Etapas** (primera sección): cuántos casos tiene cada uno hoy y **días promedio por caso que la tuvo cada uno** (con los cambios de estado registrados; el tramo desde la derivación hasta el primer cambio cuenta solo si salía de Doc. pendiente o el caso es posterior al 25/09/2026), por compañía; sin cambios registrados, una aproximación con las fechas del expediente.
*   **¿Conviene ir a mediación?** (Análisis → Compañías, `analisis.comparativaMediacion`): casos cobrados en tres grupos (arreglo sin mediación / con mediación / con juicio) con % del reclamo cobrado, días de derivación a cobro, honorarios netos y, con mediación, cuánto más que la última oferta anterior a la mediación (historial de ofertas). Filtro por compañía.
*   **Honorarios que paga cada compañía** (Análisis → Compañías, `analisis/HonorariosCompanias.jsx`): % sobre la indemnización, editable (se guarda al salir), en `pas_companias.honorarios_pct` (**SQL 23**). Vacío = el que surge de tus casos cobrados con esa compañía, o el de todas.
*   **Proyección "si todos los casos en curso salen bien"** (Análisis → Flujo de caja, al final, recuadro punteado con la marca ESTIMADO; `analisis.proyeccion`): no se mezcla con lo cobrado. Por caso en curso sin honorarios cobrados: indemnización = acordado, o reclamado × % que suele cobrarse con esa compañía; honorarios = los cargados, o indemnización × % de la compañía; comisión = la cargada o la proporción habitual; cuándo = fecha comprometida de pago, o derivación + lo que suele tardar la compañía hasta el cobro de honorarios (lo atrasado va al mes actual). Total, por mes (6 meses + después), "con tu tasa de cobro" (cobrados / cerrados) y detalle de cómo se calculó cada caso.
*   **SQL 23** (`sql/2026-09-26_23_honorarios_companias.sql`, ✅ ejecutado el 26/09): columna `honorarios_pct` en `pas_companias`.
...
### 2026-09-26 — Resumen diario en el celular (función "notificar" redesplegada el 26/09)
*   **Resumen del día** (`supabase/functions/notificar/index.ts`, `resumenDelDia`): sale con la misma tarea diaria de las 9 hs (cron `agenda_de_manana`, `{"tipo":"agenda"}`), junto con el aviso de la agenda de mañana; no hace falta SQL nuevo. Cuenta: tareas vencidas y de hoy (próxima acción con plazo), pagos en "Esperando pago" cuya fecha estimada (firma + plazo, o fecha de pago) ya llegó, mediaciones y audiencias de los próximos 7 días, casos nuevos del portal sin abrir y comisiones por pagar a PAS. Si no hay nada: "Todo al día". Una vez por día (`pas_avisos`, clave `resumen:AAAA-MM-DD`). No incluye "reclamo quieto" (depende de los márgenes que calcula la app; se ve en Para hacer).
*   **Probarlo:** "Apariencia y backup" → notificaciones → **"Mandar el resumen de hoy"** (`{"tipo":"resumen"}`, solo administrador; manda aunque ya haya salido el del día).
*   **Deploy:** hay que redesplegar la función `notificar` en Supabase con el código nuevo (panel → Edge Functions → notificar, o `supabase functions deploy notificar`). Hasta entonces, el botón de prueba no hace nada y a las 9 solo llega la agenda.

### 2026-09-26 — Historial de ofertas y contacto en la compañía (SQL 21 ejecutado)
*   **SQL 21** (`sql/2026-09-26_21_ofertas_y_contactos.sql`, ✅ ejecutado el 26/09): tablas `pas_ofertas` (cada ofrecimiento: fecha, monto, respuesta pendiente/rechazada/contraoferta/aceptada, contraoferta, nota), `pas_caso_contactos` (liquidador del caso: nombre, teléfono, mail, notas) y `pas_companias` (directorio: mail y teléfono de siniestros por compañía). **Solo administrador** (RLS `es_admin()`): el PAS y el cliente no las ven. Sin el SQL, las tarjetas avisan "falta correr el SQL 21".
*   **Ofertas** (ficha → Montos, `caso/HistorialOfertas.jsx`, `utils/ofertas.js`): lista con fecha, monto, % del reclamo, suba o baja contra la anterior, respuesta editable (con monto si es contraoferta) y nota. Cada oferta y respuesta queda en la bitácora. Mantiene los campos del caso (`camposDesdeOfertas`): `monto_ofrecimiento` = la última (la que ve el PAS); `primer_ofrecimiento` y `fecha_ofrecimiento` = la primera, solo si estaban vacíos; con una aceptada, `fecha_aceptacion` y `monto_acordado` si estaban vacíos (y el aviso de estado atrasado propone "Pasar a Esperando pago").
*   **Análisis → Compañías:** columna y KPI **"Suba 1ª → última"** (mediana; con historial usa las ofertas, sin historial el primer y último ofrecimiento del caso).
*   **Contacto en la compañía** (ficha → Resumen, `caso/ContactoCompania.jsx`): quién lleva el siniestro (nombre, teléfono con "Llamar", mail con "Escribir", notas) y, abajo, el mail y teléfono de siniestros de la compañía, que se comparten entre todos sus casos. Se guarda al salir de cada campo.
*   Las tres tablas nuevas entran en la copia de seguridad completa. `CampoMonto` ahora respeta un `onBlur` propio.
