...
*   **Aviso por mail** (pedido del usuario): cada vez que el cliente sube archivos, se manda un mail con el mismo servicio y plantilla de EmailJS que las derivaciones (`portalStorageUtils.notificarSubidaCliente`): "asegurado" = NOMBRE (DOCUMENTACIÓN DEL CLIENTE), patente, compañía y la lista de archivos por tipo, sin links (la carpeta es privada), con el recordatorio de guardarlos desde Hoy. Si el mail falla, la subida igual queda.
