...
export default function AnalisisPas({ allCasos }) {
  ...
  const columnas = [
    { k: "nombre", l: "PAS", ancho: "22%", valor: f => f.nombre.toLowerCase(), celda: f => <b style={{ fontWeight: 600 }}>{f.nombre}</b> },
    { k: "total", l: "Casos", ancho: "8%", derecha: true, celda: f => <span className="num">{f.total}</span> },
    { k: "enCurso", l: "En curso", ancho: "9%", derecha: true, celda: f => <span className="num" style={{ color: f.enCurso ? "var(--text)" : "var(--muted)" }}>{f.enCurso}</span> },
    { k: "pctExito", l: "Cobrados", ancho: "11%", derecha: true, ayuda: "De los casos ya cerrados, cuántos se cobraron", celda: f => porcentaje(f.pctExito, "var(--ok)") },
    { k: "pctDesistidos", l: "Desistidos", ancho: "11%", derecha: true, ayuda: "Desistidos sobre el total de casos del PAS (igual que en Clientes)", celda: f => porcentaje(f.pctDesistidos, "var(--bad)") },
    { k: "netoPorCaso", l: "Neto por caso", ancho: "13%", derecha: true, ayuda: "Tus honorarios menos la comisión del PAS, promedio por caso con honorarios cobrados",
    ...
    { k: "neto", l: "Neto total", ancho: "13%", derecha: true, celda: f => f.neto ? <><span className="num">{fmtMoney(f.neto)}</span><Barrita valor={f.neto} max={maxNeto} /></> : nada },
    { k: "diasACobro", l: "Derivación a cobro", ancho: "13%", derecha: true, ayuda: "Promedio de días desde que te derivó el caso hasta que se cobró",
  ...
  return (
    <>
      ...
      <section>
        ...
        <TablaAnalisis columnas={columnas} filas={filas} ordenInicial={{ k: "neto", desc: true }} clave={f => f.pasId} minWidth={860}
        ...
        <Nota>
          ...
          Neto = tus honorarios menos la comisión del PAS, de los casos donde ya cobraste los honorarios.
        </Nota>
      </section>
    </>
  );
