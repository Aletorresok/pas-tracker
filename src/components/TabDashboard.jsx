...
function GraficoCompanias({ allCasos, darkMode, cardBg, cardBorder, textColor, subColor }) {
  ...
  const stats = useMemo(() => {
    ...
    const validos1 = comp.casos.filter(c => c.fecha_inicio_reclamo && c.fecha_ofrecimiento).map(c => diff(c.fecha_inicio_reclamo, c.fecha_ofrecimiento)).filter(d => d !== null && d >= 0);
    const validos2 = comp.casos.filter(c => c.fecha_inicio_reclamo && c.fecha_cobro).map(c => diff(c.fecha_inicio_reclamo, c.fecha_cobro)).filter(d => d !== null && d >= 0);
  ...
  return (
    <div style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: 14, padding: "18px", marginBottom: 20 }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 18 }}>
        {companias.map(c => (
          <button key={c.nombre} onClick={() => setSelectedComp(c.nombre)} style={{
            padding: "5px 10px", borderRadius: 7, fontSize: 11, fontWeight: activeComp === c.nombre ? 700 : 500,
            border: `1px solid ${activeComp === c.nombre ? "#6366f1" : darkMode ? "#1e293b" : "#e2e8f0"}`,
            background: activeComp === c.nombre ? "#6366f118" : darkMode ? "#0a0f1e" : "#f8fafc",
            color: activeComp === c.nombre ? "#6366f1" : subColor,
      ...
      {stats && (
        <>
          <div style={{ display: "flex", justifyContent: "center", gap: 20, alignItems: "flex-end", height: maxBarHeight + 40, padding: "0 10px" }}>
            {barras.map(b => {
              ...
              return (
                <div key={b.label} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flex: 1, maxWidth: 100 }}>
                  ...
                  <div style={{ fontSize: 9, color: subColor }}>{b.casos} caso{b.casos !== 1 ? "s" : ""}</div>
