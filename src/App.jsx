...
function LoginGate({ onUnlock }) {
  const { T, COLORES } = useTheme(); // Obtenemos el tema
...
export default function App() {
  const { darkMode, toggleDarkMode, T, COLORES } = useTheme(); // El contexto controla el tema
  ...
  return (
    <div style={{ background: T.bg, color: T.text, minHeight: "100vh", fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{
        ...
        background: darkMode ? "rgba(9, 9, 11, 0.92)" : "rgba(250, 250, 250, 0.92)", // Adapta para el blur del tema
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                {pas.length > 0 && <div style={{ fontSize: 10, color: T.muted, marginTop: -1 }}>{pas.length.toLocaleString()} contactos</div>}
              </div>
            </div>
                ...
                {showBackupMenu && (
                  <>
                    ...
                    <div style={{ position: "absolute", right: 0, top: "100%", marginTop: 6, background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: 6, zIndex: 99, boxShadow: "0 8px 24px #0003", minWidth: 160 }}>
                      <button onClick={() => { handleBackup(); setShowBackupMenu(false); }} style={{ width: "100%", background: "none", border: "none", color: T.text, padding: "8px 12px", cursor: "pointer", fontSize: 12, textAlign: "left", borderRadius: 6 }}>💾 Descargar backup</button>
